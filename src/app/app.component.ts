...
export class AppComponent implements OnDestroy, OnInit {
  ...
  // Platform-specific logging
    ...
    // Demo routes that work without SailPoint authentication
    const authBypassRoutes = ['/visual-connector-builder', '/saas-connectivity-creator'];

    const isAuthBypassRoute = (): boolean => {
      const url = this.router.url.split('?')[0];
      return authBypassRoutes.some((route) => url === route || url.startsWith(`${route}/`));
    };
    ...
    this.subscriptions.add(
      this.connectionService.isConnected$.subscribe((connection: Connection) => {
        ...
        if (!connection.connected && !isAuthBypassRoute()) {
          void this.router.navigate(['/visual-connector-builder']).catch((error) => {
  ...
  onNavItemClick(event: MouseEvent, allowWithoutAuth = false) {
    if (!this.isConnected && !allowWithoutAuth) {
  ...
  isDemoRoute(): boolean {
    const url = this.router.url.split('?')[0];
    return url === '/visual-connector-builder'
      || url.startsWith('/visual-connector-builder/')
      || url === '/saas-connectivity-creator'
      || url.startsWith('/saas-connectivity-creator/');
  }
}
